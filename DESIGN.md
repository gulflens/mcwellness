...
The main client workspace uses a sticky section column (192px), a gap (32px) and a white content surface filling the available column with 24px padding. Enrolment uses the same proportions for its step guide and form. At a workspace container width of 767px or less, record sections become a labelled select, content padding reduces to 16px and enrolment steps use the compact grid. At container widths of 479px or less, content padding reduces to 12px and enrolment actions wrap. These are in-flow pages within the existing admin shell; saved drawer widths do not size them.
