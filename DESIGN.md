...
- Almost no motion: feedback is instant; nothing in the shell transitions.
...
### Neutral
- **Mineral Ink** (#16242a): primary text, the primary button's fill, the focus ring's outer band, and the surface colour on the dark ground. Tinted, never #111; a neutral black is a tell.
- **Second Ink** (#3e5058): secondary text (field labels, table headers, the count beside a heading, notes), placeholder text, the chevron on a select, sections that have not yet arrived in the rail, and the primary button's hover fill. The lowest tone allowed for running text.
...
- **Rule** (#cbd5d6): every hairline: table rules, the rail's edge, field borders, the line under the sign-in mark, the scrollbar thumb.
- **Cool Mineral Paper** (#eef2f1): the app ground, the rail, sticky table headers, and the primary button's text.
- **Surface** (#ffffff): inputs, secondary buttons, the active rail item, and the hover state of rail items.
...
- **H2** (500, 25px/32px): section headings within a page; balanced wrapping.
- **H3** (500, 20px/28px): the wordmark in the rail, and sub-section headings.
- **Body** (400, 16px/26px): running text, table cells, field values, buttons (at 500). Notes are capped at a 68ch measure.
- **Small** (400, 14px/22px): field labels, table headers (at 500), the count beside a heading, the Arabic name beneath a Latin one, secondary lines.
- **Micro** (500, 13px/18px, second ink): metadata such as a person's roles or the "Arriving" mark on a rail section. Sentence case, never all-caps.
- **Numeric** (tabular lining figures): applied as a class to every cell or span that holds a record number, an age, a count or a phone number.
...
**The Tabular Figures Rule.** Every numeral that could sit in a column takes `font-variant-numeric: tabular-nums lining-nums`. Record numbers, ages, counts and phone numbers align because of the figures, not because of a monospace fallback.
...
The spacing scale is a 4px step: 4, 8, 12, 16, 20, 24, 32, 48, 64. Twelve is the horizontal unit inside rows, rail items and inputs; sixteen is the gap between siblings; twenty-four is the gap between blocks; thirty-two is the page's outer padding.

Density is set by one number: every row, input, button and rail item is 44px tall. The tap target rises to 48px for the sign-out control and, by the brief, on the instrument. Sign-in and the plain landing pages are a single centred column, 24rem wide (28rem on the instrument, the 68ch measure on the record), padded 64px above and 24px at the sides.

There is one breakpoint, at 720px. Below it the grid collapses to one column; the rail becomes a static band under a hairline, laid out as a two-row grid with the wordmark and the person on the first line and the sections wrapping on the second; not-yet-arrived sections and the roles line are hidden; the page pads 24px above, 16px at the sides and 48px below; the toolbar fields go full width; and the table scrolls inside its own container with the first two columns (the record, 7.5rem wide, and the name) pinned on the inline start, the name column drawing an inset hairline on its end edge. The page body never scrolls sideways.

### Named Rules
**The 44 Rule.** Rows, inputs, buttons and rail items are 44px tall. One height sets the ledger's rhythm; the sign-out control and the instrument's controls take 48px.
...
**The Hairline Rule.** Structure is drawn with 1px rules in Rule (#cbd5d6): table rows, the rail's edge, field borders. No zebra striping, no card borders, no boxes around groups.

## Elevation & Depth

The system is flat. No surface in the shipped app carries a shadow at rest or on interaction; depth is conveyed by tone (surface white on paper for inputs, buttons and the active rail item), by hairline rules, and by the sticky positioning of the rail and the table header. A single soft shadow token (`0 8px 24px` of ink at 14%, deeper on the dark ground) is defined in `app/shell/tokens.css` and reserved for the detail drawer and menus, which are not yet built; nothing references it today.

The focus ring is the one treatment that lifts an element: a 2px band of paper inside a 2px band of ink (`0 0 0 2px paper, 0 0 0 4px ink`), on a 4px radius, applied to any `:focus-visible` element. It reads on both grounds because both bands swap with the ground.

### Named Rules
**The Flat Ledger Rule.** Surfaces are flat. Depth is tone and rule, never a drop shadow. The one shadow token exists for the drawer and menus and is not used until they arrive.

## Shapes

Rectilinear with a single softening. Every rounded corner in the app is 4px: inputs, buttons, rail items, the seeded-person buttons on sign-in, and the focus ring. The 2px and 8px steps exist in the token file and are unused. The only circle is the 6px status dot (filled in a status hue or slate; hollow, as a hairline ring in second ink, for a lead). Icons are drawn on a 20px grid with a 1.5px stroke, round caps and joins, in `currentColor`, and are always marked `aria-hidden`. Borders are hairlines (1px) in Rule, except the secondary button, whose hairline is in ink. Nothing is clipped, tilted or masked.
...
- **Secondary (default):** surface fill (#ffffff), ink text, a 1px ink border. Hover tints the fill with 6% ink (#f1f2f2); press tints it with 10% (#e8e9ea).
...
- **Name cell:** the Latin name, and beneath it the Arabic name in small second ink with `lang="ar"` and `dir="rtl"`.
...
- **Don't** add hover transitions, entrance animations, skeleton shimmer or page-load orchestration; feedback is instant and nothing in the shell transitions.
