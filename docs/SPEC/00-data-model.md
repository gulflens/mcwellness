...
One row per answer a household gives the portal's review line (`SPEC/client-portal.md` sections 3.1 and 6.6; added 2026-09-17, migration 704). `client_id`, `contact_id` (who answered), `milestone_kind` (`brain_map` | `package_complete`), `milestone_id` (the completed brain-map appointment or the finished package purchase; a plain uuid, not a foreign key, so the answer outlives neither row), `outcome` (`opened` | `dismissed`, read by the narrative sentence and nothing else); `created_at` is when the answer was given. Unique per client and milestone, whichever adult answered, and the policy binds `contact_id` to the person signed in.
...
status              'scheduled' | 'in_progress' | 'completed' | 'no_show' |
                    ...
                    'voided'                     -- 2026-09-23: a records row withdrawn as logged in error
...
voided_at, voided_by, void_reason                -- 2026-09-23: the void's stamp; all three or none, only on a voided row
```
...
_Amended in trunk round 60, 2026-09-23 (migrations 969 and 970, `docs/superpowers/specs/2026-09-23-void-logged-session-design.md`)._ A `records` row logged in error is **voided, never deleted and never edited**. A void is a stamp on a closed record: the status becomes `voided` and `voided_at`, `voided_by` and `void_reason` are filled — all three or none, a reason that is more than whitespace, and only ever on a `voided` row — while the date, the service, the practitioner and the length stay exactly as logged. Only a `records` row may be voided; a `device` row is a visit the household had, and unwinding one is a credit note (`billing.md` section 4.3), unbuilt. The close guard (`app.session_refuse_update_after_close`) admits exactly one change to a closed row, `completed` to `voided` on a `records` row with every other column standing still, and only while `app.void_active` names this session in this transaction — a marker only `app.void_recorded_session` writes, so that function is the one door. A voided row keeps its `closed_at` and is refused every further change. The credit the visit consumed comes back the way a waiver gives one back (`entitlement` below). A **correction** is the void and a new visit in one act: the new row is version 2 of the wrong one, with `supersedes_id` pointing at it and the office's reason as `amendment_reason` — the first real use of those columns. _Amended 2026-09-23, as the final reviews of round 60 found (migrations 970 and 971):_ the stamp and the status go together both ways (`voided` only with the stamp, the stamp only on `voided`), and `voided` only on a closed `records` row, by check; and a row becomes `voided` — on insert or on update, open or closed — only while `app.void_active` names it, which a second guard (`app.refuse_void_without_marker`, `enable always`, on `session` and `appointment` both) enforces, so the one door is the only door for every writer. `supersedes_id` is bound to the household as well as the practice: `(tenant_id, client_id, supersedes_id)` references `session (tenant_id, client_id, id)`, so a correction stays with the household its first version was logged for. An erasure replaces `void_reason` with the fixed phrase `amendment_reason` already takes ("Erased with the record"), on the session and on its appointment; the credit's `waiver_reason` is a financial record and is kept.
...
_Amended in trunk round 60, 2026-09-23 (migrations 969 and 970, `docs/superpowers/specs/2026-09-23-void-logged-session-design.md`)._ `status` gains `voided`, and the row gains `voided_at`, `voided_by` and `void_reason` on the same terms as `session`'s: all three or none, and only on a `voided` row. An appointment is voided only with the `records` session that fulfils it, by `app.void_recorded_session`, never on its own — enforced since the final reviews of round 60 (migration 970) by `app.refuse_void_without_marker`, which refuses any insert or update to `voided` unless `app.void_active` names the session that points at this appointment in this transaction, and by a check that `voided` and the stamp go together both ways; the stamp is the whole change. An erasure replaces its `void_reason` with "Erased with the record" (971). A voided appointment no longer holds its window — the two exclusion constraints ignore it beside `cancelled`, `cancelled_late`, `no_show` and `rescheduled` — so the right visit can be logged at the same hour.
...
- **The books** (`accounting.md`, migrations 450–454): `accounting_setting`
  ...
  (the chart: four-digit `code`, `type`, an optional unique `role` the poster
  finds accounts by); `fiscal_year` (a range and an `open`/`closed` status);
  `journal_entry` and `journal_line` (balanced, append-only,
  `unique (tenant_id, source_table, source_id, source_event)` as the poster's
  idempotency). None carries a `client_id`, by design: statements name nobody
  and an erasure reaches nothing here.

---

## 7. Cross-cutting rules

**Append-only records.** `session` (once closed), `report` (once signed), `client_protocol`, `assessment`, `invoice` are never updated in place. Each carries `version`, `supersedes_id`, `amendment_reason`. The current version is the one with no successor. Full snapshots, not diffs. `journal_entry` and `journal_line` (migration 453) are never updated or deleted either, for any role including the database owner's; they carry no version because a correction is not a new version of an entry but a new entry that reverses it (`accounting.md` section 4.2).
...
5. ~~**Setup photo**~~ — **retired 2026-09-09.** The practice takes no photographs (its legal advisor's recommendation); the routes, the camera and the filing functions are gone. `session.setup_photo_document_id` remains as read-only history, because the erasure function unlinks it by name.
...
- **Piece eight's spec (2026-09-05, `SPEC/practitioner-phone.md`):** `kit` is created at last (migration 306) with the rule at check-in and `session.kit_id` set from it; `drive_estimate` joins section 5 as scheduling's cache behind the routing seam (`docs/SEAMS.md`), with `scheduling_setting.drive_road_factor` and `drive_peak_multiplier` as the fallback's figures; the setup photograph's bytes got their door and were filed under the seam's own client key, and the closed-visit guard of migration 302 admitted exactly the one link `app.file_setup_photo` wrote. **Both are retired as of migration 960 (2026-09-09):** the door is deleted, the function is dropped, and a closed visit now admits no change at all.
- **Shared zone, round 35 (2026-09-07): the books (piece eleven, `SPEC/accounting.md`).** A new `accounting` stream keeps the practice's own general ledger inside the platform, with migrations 450–454 and the range `450–499` carved from billing's upper half. Four actions join `domain/shared/actor.ts` — `accounting.read` and `accounting.write` for the owner and finance, `accounting.year.close` and `accounting.settings.write` for the owner alone — with `canOpenBooks` in `app/shell/adminAccess.ts`, a Books entry in the rail, `/admin/books` in `App.tsx` and `mountAccounting` in `create-api.ts`. Nothing in billing changes: the poster reads through `app.unposted_money_events()` (454, security definer in 952's pattern), which steps past the erasure gate on purpose so what the books say never depends on who ran it, and writes only accounting's own tables. `app.bootstrap_practice` learns about the two new per-practice defaults in the trunk's `958_bootstrap_knows_the_books.sql`, because a merged migration is never edited. `domain/shared/audit-narrative.ts` gains the five tables' sentences, including which way the books' lock moved, and `package.json` gains `pnpm job:post-books`, the nightly catch-up. The books name nobody: no table here carries a `client_id`, so `app.erase_client` gains no step and the confirmation letter no sentence.

## Enquiry (added 2026-09-09, migration 916)
