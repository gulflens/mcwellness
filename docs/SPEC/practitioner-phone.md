...
**3.1 The manifest and the icon.** *(Amended 2026-09-12: the manifest now opens
at `/` rather than `/today`, carries no orientation, and names three PNG icons
beside the SVG — see `docs/SPEC/desktop-install.md`. The practitioner is
unaffected: `/` sends each person to their own home by role, which for her is
this day sheet. The paragraph below describes the manifest as first written.)*  `public/manifest.webmanifest` (name, `start_url` `/today`, `display` `standalone`, portrait, the practitioner ground's paper colour as the theme, copied verbatim from `app/shell/tokens.css` with a comment saying so — and that comment names all three places a token's value is written out, the manifest, `public/icon.svg` and the Static Maps style in `app/api/_middleware/routing/google.ts`, since none of the three can read a custom property) and `public/icon.svg`, achromatic, one glyph, no accent, exactly as change request 02 section 1a and 1b drafted them. `index.html` links the manifest and the same file as `apple-touch-icon`. A raster iOS icon is a binary asset the operator may supply later; its absence costs polish, not installability.
...
> **Retired 9 September 2026.** The practice takes no photographs, on its legal advisor's recommendation; the capability went with the wording in the same round (`db/migrations/960_retire_the_setup_photograph.sql`). What is written above is kept as the record of what was built, not as an instruction to build it again. `session.setup_photo_document_id` survives and is read-only: a visit photographed before that date still names its picture, and the erasure function unlinks it by name.
...
From piece seventeen (2026-09-07, `docs/SPEC/route-planning.md`) the **console** has an interactive map, on its own document with its own security policy and a browser key of its own. Today keeps this static picture, unchanged: it is what works with no signal, on a device that should not be loading a third-party script down a lift shaft, and decision 2 stands.
