...
Status: **approved by the operator on 2026-09-06 with the plan page** (`docs/PLAN/piece-ten.md`), the defaults marked as Claude's standing until overruled. **Amended in the build, 2026-09-06, in two places and no others, each marked where it stands:** section 5, by trunk round 29 — the writer's byte-level half moved to `domain/shared/document` and the sending seam to `domain/shared/sending.ts`, so the sentence describing that as an unsolved filing problem no longer describes anything; and section 6's `report` table, by the fix round — the recipient block (`recipient_name`, `recipient_record_number`) joined the snapshots, because reading it live at render made the byte-identical re-render of section 11 hold only until a household corrected the spelling of a name. Nothing else in this file was touched by the build: the session report's own form, which the fix round built, needed no amendment — sections 1, 4.2 and 5 scoped both kinds all along and section 9 never excluded it. Written 2026-09-05. **It replaces the draft of 2 September 2026**, which was written before the PDF writer, the document filing, the sending seam, the portal and the practice's own identity existed. Where the two differ, this file stands. The decisions the draft raised are re-answered in section 10.
...
| --- | --- |
| A pure bilingual PDF writer, `domain/shared/document` with billing's own wording beside it: the page renderer (`pdf.ts`), the TrueType reader (`truetype.ts`), the Arabic shaper (`arabic.ts`) and the extractor the tests read a finished page back with (`extract.ts`), plus, in `domain/billing/document`, the invoice's own model, layout and wording. `renderDocument(document, fonts)`; nothing in it reads a clock or a database | The report's own model, layout and wording over the same writer, imported from `domain/shared/document` (section 5) |
...
The writer is good and nothing here is rewritten. There was one honest problem and it was a filing problem: the writer lived under `domain/billing`, and `OWNERSHIP.md` rule 3 forbids one module importing another module's `domain/`. **_Amended in the build, 2026-09-06 (trunk round 29):_ that move is done.** The byte-level half — `pdf.ts`, `truetype.ts`, `arabic.ts`, `extract.ts` and their tests — is `domain/shared/document`, which this worktree imports directly; `model.ts`, `render.ts` and `strings.ts` stayed with billing where the invoice's own wording belongs. The sending seam is `domain/shared/sending.ts`, its implementations under `app/api/_middleware/sending/` in the routing seam's shape. The same fixture invoice and receipt rendered to the same bytes before and after, so nothing about a filed document changed.
...
| `signed_by_name`, `signed_by_certification`, `signed_by_certifying_body`, `signed_by_certificate_number`, `recipient_name`, `recipient_record_number`, and the practice identity columns beside them | The snapshots of section 3. **_Amended in the build, 2026-09-06 (the fix round):_** the recipient block joined them. Read live at render it made the byte-identical re-render hold only until a household corrected the spelling of a name, which section 9 rules out in as many words. |
...
**7.3 What the household sees.** Issued reports for their own client, and nothing else: never a draft, never a superseded version they were not sent, never another household's, never the electrode sites, never the raw assessment figures except as the report quotes them. **_Amended in trunk round 31, 2026-09-06 (the operator's decision, reversing default 4 of pull request 83):_** "their own client" means a client they are a **legal guardian** of, or themselves once they are an adult — a young person's own portal login reads no report about themselves, because a report is a practitioner's written summary of a person and a guardian is who receives it and talks a child through it; `app.actor_may_read_reports_of` (migration 955) is what says so, in the row policy rather than on a screen. On the portal a sixth screen lists them by reference, kind, coverage and date, each opening through the same short-lived audited link the money screen already uses for an invoice. That screen lives in `app/client/**`, which is `client-portal`'s; this spec says what it shows and that stream builds it.
