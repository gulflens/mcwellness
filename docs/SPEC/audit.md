...
12. Migration `098_erasure_guard.sql` (shared-zone round 3, 2026-09-02) replaces the role check in item 10's last sentence: it held only while nothing erased through the API, and stops holding once `app.erase_client` (client-record.md section 8) is a security definer function the API calls under `app_role`. `app.audit_redact`'s erasure branch now compares `app.erasure` against a secret held in `app.erasure_key` (one row, 32 random bytes, no grant to `app_role` or `public`) instead of checking which role is assumed. `app.begin_erasure()` — itself security definer, granted to nobody — stamps the setting from that table; only a security definer function sharing its owner, `app.erase_client`, can call it, because Postgres lets same-owner functions call one another without an explicit grant. A request that sets `app.erasure = 'true'` by hand, forging the old flag, now withholds nothing: `'true'` never matches the encoded key.
