...
*Amended 2026-09-10 (trunk round 43, the walk's fixes):* a read carries no reason. `app.reason` is stamped on every audit row a request writes, so a read made while, say, a move drawer opened was carrying the move's own reason and implying the read explained itself; the reason is now shown only beside a change. Identical reads by one person inside one minute are shown once, with a count, rather than as one line for every glance.
