...
| `billing` | `domain/billing/**`, `app/admin/billing/**`, `app/api/billing/**`, `db/policies/billing/**`, `jobs/billing/**`, `tests/billing/**` | `400–449` | `SPEC/billing.md` |
...
> Widened for the trunk's round 31 (2026-09-06, `docs/CHANGE-REQUESTS/trunk-notes.md`): the trunk answered, in one round, what pieces nine and ten left at its door and four of the small things in `docs/PLAN/pieces-seven-to-nine.md`. Nine items, listed there with every default taken. For that round only, the trunk edited five streams' paths, each file named in the round's own "every file this round touched outside the trunk's own paths": `assessment` — `app/api/assessments/routes.ts`, `rows.ts`, `schema.ts` and `file.ts`, `app/admin/assessments/RecordDrawer.tsx`, `AssessmentsTab.tsx` and `AssessmentsTab.test.tsx`, `domain/assessment/fileType.ts` and its test, and migration `db/migrations/502_assessment_document_key.sql` in the stream's own range; `client-record` — `domain/client/index.ts` and `domain/client/fileSignature.ts` with its test, which moved to `domain/shared`, and `tests/client/db/erasure_act.test.ts`; `client-portal` — `tests/portal/db/invite.test.ts`; `reports` — `tests/reports/document.test.ts`; `billing` — `app/api/billing/summary.ts` and `tests/billing/db/packages.test.ts`; and `audit-ui` — `app/api/audit/activity.ts` and `schema.ts`, and `app/admin/audit/AuditPage.tsx`, `AuditPage.test.tsx` and `audit.css`, which build section 9's activity feed and access report and which that stream owns from here. Nothing in those paths is the trunk's beyond this round.
>
> The round's **fix round** (2026-09-06, the same widening) added seven files to that list, three of them because two decisions the operator took at 06:15 landed inside it: `reports` — `db/policies/reports/reports.sql`, `tests/reports/db/portal.test.ts` and `tests/reports/db/rls.test.ts` (a fixture moved so the seeded mother is a guardian); and `client-portal` — `app/api/portal/reports.ts` and `app/api/portal/household.ts`, with `domain/portal/reports.ts`, its test and `domain/portal/index.ts`. All of them carry one rule, that a report about a young person is read by a legal guardian or by the person themselves once they are an adult (`docs/CHANGE-REQUESTS/trunk-notes.md`, round 31's fix round, section 4). The fix round also changed one word in a comment in three `assessment` files already listed above, and nothing else outside the trunk's own paths.

> Widened for the qa-fixes-1 round (2026-09-06, `docs/CHANGE-REQUESTS/qa-01.md`): the five product defects the browser walk of 6 September recorded span four streams, so — as the trunk's round 31 did, and by the integrator's decision under the cost rules of `docs/HANDOVER.md` section 6 — one branch answered all five rather than four branches answering one each. For that round only, the files touched outside a single stream's own paths were: `scheduling` — `domain/scheduling/status.ts`, `cancellation.ts`, `index.ts` and the two tests beside them, `app/api/appointments/confirm.ts` (new), `routes.ts`, `schema.ts`, `cancel.ts` and `move.ts`, `app/admin/schedule/SchedulePage.tsx`, `CancelAppointmentDrawer.tsx` and `MoveAppointmentDrawer.tsx`, and `tests/scheduling/db/move_and_cancel.test.ts`, `SchedulePage.test.tsx` and `MoveAndCancelDrawers.test.tsx`; `reports` — `app/api/reports/gather.ts` and `tests/reports/db/reports.test.ts`, with `domain/reports/document/strings.ts`, `render.ts` and `index.ts` and `tests/reports/document.test.ts` (the round's own fix round, which put the comparison's own sentence beneath the figures it had just made print); `client-record` — `app/api/clients/list.ts` and `tests/client/db/list_pagination.test.ts`; and the trunk's own `domain/shared/audit-narrative.ts` with its test. No migration and no policy file: every rule the round added is enforced where it is written and tested, and the one place a database function might have repeated it is argued in the change request's own "Left standing, deliberately". Nothing in those paths is that round's beyond it.

> Widened for the fee-not-session round (2026-09-06, `docs/CHANGE-REQUESTS/billing-05.md`): the founder's decision of 4 September — one fee, never a session — is one rule that spans two streams, because the scheduler decides a visit is cancelled and the ledger decides what that costs. So, as trunk rounds 31 and 32 did and by the integrator's widening for one round, one branch answered both rather than two branches answering half each. For that round only, `billing` also owned: `domain/scheduling/cancellation.ts` (its comments, which described a consumption that no longer happens), `app/api/appointments/cancel.ts` and `app/api/appointments/schema.ts` (the cancel response now names the charge instead of the credit), `app/admin/schedule/CancelAppointmentDrawer.tsx` and `CancellationPolicyDrawer.tsx` (the words a coordinator reads, and the waiver the drawer offers), and `tests/scheduling/db/move_and_cancel.test.ts`, `tests/scheduling/MoveAndCancelDrawers.test.tsx` and `tests/scheduling/CancellationPolicyDrawer.test.tsx`; plus `docs/SPEC/billing.md` section 4.3 and `docs/SPEC/scheduling-manual.md` sections 3 and 6.4. Rule 3 was not widened and did not bend: `app/api/appointments/cancel.ts` reads the charge back out of the ledger rather than importing `callOutFeeFor`, and the cancel drawer reaches that rule through `app/admin/billing/money.ts`, which re-exports it as it already re-exports `formatFils`. Nothing in those paths is billing's beyond this round.

> Widened for the trunk's round 33 (2026-09-06, `docs/CHANGE-REQUESTS/trunk-notes.md`): the trunk answered, in one round, the five requests the fee round left at its door (`docs/CHANGE-REQUESTS/billing-06.md`) and the two the assessment stream's file door left (`docs/CHANGE-REQUESTS/assessment-02.md`, requests 2 and 3). Seven items, listed there with every default taken. Three of them fall outside the trunk's own paths, so — as rounds 31 and 32 did, and by the integrator's widening for one round — one branch answered all seven. For that round only, the trunk edited three streams' paths, each file named in the round's own "every file this round touched outside the trunk's own paths": `client-portal` — `app/api/portal/schema.ts` and `app/api/portal/money.ts`, `app/client/MoneyScreen.tsx` and `app/client/i18n/dictionary.ts`, and `tests/portal/fixtures.ts`, `tests/portal/screens.test.tsx`, `tests/portal/dictionary.test.ts` and `tests/portal/db/routes.test.ts`, which put a forgiven fee on the household's own money screen; `assessment` — `domain/assessment/fileType.ts` and its test, which drop the stream's local EDF check and re-export `domain/shared`'s under the same name so no caller moved; and `scheduling` — the new migration `db/migrations/205_unfit_fee_is_the_call_out_fee.sql` **in that stream's own range**, which replaces migration 202's comment on `scheduling_setting.unfit_fee_fils`, and `app/api/appointments/settings.ts`, whose doc comment still quoted the cancellation rule the founder ended. The trunk's own half is migration `957_vat_taxable_supplies_excludes_waived.sql` in the `950–999` half, `domain/shared/fileSignature.ts` with its test, `domain/shared/index.ts`, `domain/shared/audit-narrative.test.ts`, `tests/db/`, `docs/SPEC/` and `docs/CONSENT/`. Migration 205 is the shape to copy for a migration that only comments another stream's column: it sits in that stream's range and its `-- Needs:` names 202 alone, because a comment describes rather than depends and `checkNeeds` refuses a `Needs` at or above a file's own number. `docs/SPEC/00-data-model.md` is deliberately **not** edited: it lists neither the function nor the column comment. Nothing in those paths is the trunk's beyond this round.

> Widened for the trunk's round 34 (2026-09-06, `docs/CHANGE-REQUESTS/trunk-notes.md`): the trunk answered, in one round, the four requests already written that `docs/HANDOVER.md` section 10 step 3 owed to a later round — `qa-01.md`'s Arabic vocabulary for the five bands, `reports-01.md`'s R3 (the colour operator in the shared PDF writer) and R4 (the read-audiences page), and the "one press" to the access report that round 31's fix round left at the audit stream's door — plus one item that closes with a reading and builds nothing (the household's own step in confirming a booking, which `docs/SPEC/scheduling-manual.md` names none of). Five items, listed there with every default taken. Four of them fall outside the trunk's own paths, so — as rounds 31, 32 and 33 did, and by the integrator's widening for one round — one branch answered all five. For that round only, the trunk edited five streams' paths, each file named in the round's own "every file this round touched outside the trunk's own paths": `assessment` — `domain/assessment/types.ts` and `validateDerived.test.ts`, and `app/admin/assessments/copy.ts` with its new `copy.test.ts`, which re-export and derive the five band names from the shared vocabulary rather than holding a copy; `reports` — `domain/reports/types.ts` and `gatherProgress.test.ts`, `domain/reports/document/sheet.ts` and `render.ts` (the ribbon's slices now carry the band's hue), `app/api/reports/gather.ts` (which fills `labelAr`), and `tests/reports/document.test.ts` and `tests/reports/db/reports.test.ts`; `audit-ui` — `app/admin/audit/RecordTimeline.tsx` and `AuditPage.tsx` with both their tests, which build the one press; `client-record` — `app/admin/clients/ClientDrawer.test.tsx` alone, a `MemoryRouter` around its three renders, because the timeline it mounts now renders a `Link`; and `session-capture` — `domain/session/events.ts` and `events.test.ts`, where the same five band keys stood a fourth time and are now the shared vocabulary re-exported under this folder's own `BAND_KEYS` and `BandKey`, so that the five have one home in fact and not only in the two modules the request named. No caller in that stream moved. **`app/admin/clients/ClientDrawer.tsx` is not edited**: the press lives in `RecordTimeline`, which is where round 31's fix round said it belonged. The trunk's own half is `domain/shared/bands.ts` with its test, `domain/shared/index.ts`, `domain/shared/document/pdf.ts` with its test, `app/shell/shell.css`, `tests/security/xss.test.tsx`, `docs/SPEC/audit.md`, `docs/SECURITY.md` and `docs/CHANGE-REQUESTS/`. **No migration and no policy file**: nothing this round decided is a database's to enforce, and `docs/SECURITY.md`'s new "Who may read what" documents policies that did not change — that section says in its own words that it is rewritten in the same pull request as any policy that does. `docs/SPEC/00-data-model.md` is deliberately **not** edited: it lists neither a vocabulary module nor a content-stream operator. The round stacks on round 33 and its pull request is opened against `trunk-round-33`, because both edit the same record files. Nothing in those paths is the trunk's beyond this round.
