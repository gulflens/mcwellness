...
**3.1 Check in.** Tap at the door. The location is recorded only when the practitioner switches sharing on — as proof of attendance, read only by the practice, following the session's own retention (section 7 has the full rule). Blocks if, as shipped: participation consent is missing; a minor's guardian consent is missing; home-visit consent is missing; the practitioner's certification is not valid today; the client's date of birth is not on file; the practitioner is already checked in elsewhere. Each block names the reason and who to call. Works offline using cached consent/credential state as of last sync, with a visible "verified at HH:MM" note.
