...
**Amended 2026-09-13 (trunk round 49).** The export from the practice's own brain-mapping and neurofeedback software is attached here (`ExportStep.tsx`; section 3.3's amendment; `PUT /api/sessions/:id/export`, migration 307): the practitioner is standing over the laptop the file was just written on, finishing the visit, the one moment the export and the record are in the same place. It is optional and never a gate — `domain/session/canCheckIn.ts` gates entry and nothing here gates exit, so a visit with no export still closes on the same button. It cannot be attached once the visit is checked out: migration 960 took away the one change a closed visit admitted, and the door answers a closed visit with a refusal rather than a silent no-op. Because the dock is sticky and the export's own "no export attached" marker is not — it can scroll out of view while Check out never does — the dock itself carries a line of note text when none is attached, and the button asks twice before the second tap closes the visit for good, the same arm-then-confirm the Run step already uses for the same reason: what is on the other side of that tap cannot be undone.

**Amended 2026-09-13 (trunk round 49).** "Signal score" above is the same staleness sections 3.3 and 3.4 were amended for: `SummaryStep.tsx` shows the "Signal at setup" and "Session quality" rows only while `tenant.record_readings` is true, because with the practice's own software taking every reading now, no figure is ever recorded on a readings-off visit to show. A visit that already holds readings from before the switch was turned off keeps showing them regardless of the switch's current value, exactly as sections 3.3 and 3.4 already say of the screens ahead of this one — nothing that displays a score reads the switch, only `stepsFor` does. The sentence above is left as it was written rather than rewritten away, because a reader reviving the readings needs to know what the summary showed.
...
*Amended 2026-09-23 (trunk round 60, `docs/superpowers/specs/2026-09-23-void-logged-session-design.md`, migrations 969 and 970): a visit logged from the records in error is voided or corrected, never deleted and never edited. `POST /api/sessions/:id/void`, with a reason, calls `app.void_recorded_session`, the one thing that may void a session: the session and its appointment are stamped `voided` with when, by whom and why, every fact they hold stays as logged, the window is freed, and the credit the visit took comes back the way a waiver gives one back — a visit settled before the app took none and gets none. It refuses a `device` row (a visit the household had; unwinding one is a credit note, unbuilt), a row not completed, a row already voided, and a row a measurement, an invoice or a billing question still names, each with its own code. A correction is the same void and a fresh log in one act: `POST /api/sessions/from-records` with `replaces` voids the wrong visit and writes the right one as its version 2 — `supersedes_id` the wrong one, `amendment_reason` the reason — inside one savepoint, so the wrong visit is never gone while the right one is missing. This is the amendment path this section names, for `records` rows only. The same three roles may do either.*
...
*Amended 2026-09-23 (trunk round 60): a void is the sensitive action `session_voided`, carrying the reason, against the visit withdrawn; a correction writes it against the old visit and `session_recorded_from_records` against the new one. Every refusal on the way is `refused` against the visit's id, and the Timeline says why in words, never by its raw code (a code it has no phrase for reads as a refusal with no reason given).*
...
*Amended 2026-09-23 (trunk round 60): correcting such a visit is now built — section 4's second amendment. Still out: a note or ratings on one, a void or credit note for a visit closed on the phone, and editing a logged visit's fields in place.*
