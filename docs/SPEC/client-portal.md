...
_Amended 2026-09-17 (the owner's decision of 16 September 2026, trunk round 52, `docs/superpowers/specs/2026-09-17-review-prompt-design.md`):_ between what is waiting on the household and how to ask for a visit, **the review line**, when there is one. One sentence — "If you would like to, you can leave us a review on Google." — and two answers, "Leave a review", which opens the practice's own review page (`tenant.review_url`, section 6.4) in a new tab with no referrer, and "Not now". No heading, no colour, no motion. It appears after one of two milestones, decided by `reviewMilestones` (section 5, rule 9): a completed brain-map visit, or a package whose last credit has been used, each within the last ninety days; once per milestone, at most one line per client, and only for a client this person is shown money for, so never to a young person's own login. Either answer writes one `portal_review_prompt` row (section 6.6) and the line is never shown again for that milestone. Where the practice has recorded no review page, nothing is computed and nothing is shown.

**3.2 Visits** (`/portal/visits`). Upcoming: appointments with status `confirmed` or `checked_in` from the start of today, earliest first, each with the date, the 45-minute window as "10:00 to 10:45", the service's name and whether it is at home or the studio. Past: `completed`, `no_show`, `cancelled` and `cancelled_late`, most recent first, with the date and the service and a word for the outcome (`cancelled_late` reads as "Cancelled" to the household; the fee, if any, is the money screen's business). `proposed` is not shown — the practice has not told the household yet — and `rescheduled` rows are not shown because the visit that replaced them is. Never the practitioner's name, never a note, never a coordinate. _Amended 2026-09-23 (trunk round 60):_ a `voided` row is not shown either — a visit the office logged from its records in error and withdrew is not a visit the household had. When the office corrects one, the right visit is listed in its place; the credit given back appears on the money screen as available, because that screen reads credits by status.
...
**One exception to "no nudges", decided by the owner on 16 September 2026 and built in trunk round 52:** the review line of section 3.1. It is an offer rather than a nudge on four conditions, each of which is the rule: it is shown once per milestone and never repeated, it is dismissable in one press, it stacks with nothing, and it is one sentence in the tone above with "if you would like to" as its first words. A second line after a "not now", a reminder by any channel, a star widget inside the portal, or any wording that asks twice would be the nudge this section forbids, and is not admitted by this exception.
...
9. `reviewMilestones({ visits, purchases, entitlements, answered }, today)` in `domain/portal/reviewPrompt.ts` (trunk round 52, 2026-09-17): the milestones the review line may be offered for. A brain map is a completed visit whose service carries the code `brain-map`; a finished package is a purchase whose `packageProgress` reads used equals total, above nought, reached on the day its last credit was used. Within `REVIEW_PROMPT_DAYS` (ninety) of `today`, not yet answered, and at most one per client, the most recent. `portal.review.answer` (new, `domain/shared/actor.ts`): a `client_contact` for a client in `clientIds`, the shape of rule 3's first half.
...
_Amended 2026-09-17 (trunk round 52):_ **migration `920_practice_review_url.sql`**, the same range and the same shape as 912's `website`: `tenant.review_url text` nullable, a URL with its scheme by check. Shown on the Practice settings page beneath the website and written by the practice route — and, unlike the three footer fields, written even when the value sent is null, because clearing it is how the review line is switched off for every household.
...
_Amended 2026-09-17 (trunk round 52):_ `db/policies/portal/access.sql` gains `portal_review_prompt`: tenant isolation; select for owner, admin, lead practitioner, and a contact for their own client; insert for a contact for their own client and nobody else, the office included — a coordinator who wants the line gone clears the review link; no update and no delete policy because migration 704 grants neither verb. The erasure gate as on the other two.
...
| `POST /api/portal/review-prompts` | the household's answer to the review line (section 3.1, amended 2026-09-17): `{ clientId, milestoneKind, milestoneId, outcome }`; `portal.review.answer`, and only for a client this person is shown money for; one row per milestone, a second answer idempotent; `{ ok: true }` |
...
Invite issuing (`POST …/invite`): if the contact has no `app_user`, creates one (`display_name` from the contact's names, `preferred_locale` from the client's, status active) with a `client_contact` role and writes `contact.user_id`; if one exists and is suspended, reactivates it; issues a 32-byte random token, stores its sha256, kind `first_sign_in` when `auth_id` is null and `password_reset` otherwise, expiry seven days; answers the link and the message. Revoke: `app_user.status = 'suspended'` (so `app.resolve_actor` refuses the next request and the shell signs the person out) and `revoked_at` on every open invite. Both write `logAction` rows naming the contact's id and nothing else. **For a contact whose account also works at the practice** (trunk round 59, `docs/CHANGE-REQUESTS/trunk-round-59.md`), Revoke ends the link and not the account: `app.unlink_household_contact` (migration 968) nulls `contact.user_id` and drops the account's idle `client_contact` role when no other contact still links to it, the open invites are closed the same way, the trail row carries `means: unlinked` (a household's carries `means: suspended`), and the answer is `state: none`. Invite refuses such a contact with `409 not_a_household` as before, and the screen names that refusal instead of asking for a retry.
...
Every open of a portal screen writes `read` or `list` rows as the contact, so the practice's timeline shows the household's own visits to the record. Invites, revocations, requests and handling are `logAction` rows or row-trigger rows with ids only. An answer to the review line (2026-09-17) is a row-trigger row and a `portal.review.answered` action carrying the milestone's kind and which way the household answered, ids only; an answer from a young person's own login, or for a client outside the household's money, is a `refused` row. The narrative catalogue (`domain/shared/audit-narrative.ts`) gains sentences for `portal_invite`, `portal_request` and the two actions, in both languages; the generic branch already keeps the timeline whole until it does.
