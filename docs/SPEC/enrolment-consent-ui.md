...
The record has a “Back to clients” control, client heading, record number and status. Desktop section navigation is a sticky 192px column beside a white content surface filling the available column, with a 32px gap and 24px content padding. Vertical tabs support arrow keys and Home/End. At a workspace container width of 767px or less, sections become a labelled native select, the layout uses one column and content padding reduces to 12px. Finance and other role restrictions remain unchanged; an unavailable requested section falls back to Overview.
...
At a workspace container width of 767px or less, progress becomes a four-column grid, step numbers disappear, fields become one column, record and enrolment content padding reduces to 12px, consent introduction and selection padding reduces to 16px, and enrolment actions wrap. These are layout rules, not a claim of complete device or browser coverage. Enrolment progress and unsaved fields are not restored from the URL after reload; existing saved lead records remain governed by the existing persistence rules.
...
Synthetic records were reviewed within the actual `AdminLayout` at 1100px desktop and 390px phone widths. Captures are `.impeccable/review/workspace-consent.png`, `.impeccable/review/workspace-signing.png` and `.impeccable/review/workspace-enrolment.png`. The independent reviewer disposition was ship for this bounded scope. The focused checks passed 35 tests. The final full suite passed 3,184 tests; earlier verification counts above apply to their earlier refinements. These captures do not establish live-backend, exhaustive role, locale or device coverage.

The responsive follow-up removed the 1040px content cap and was reviewed at browser viewport widths of 1920px, 1360px, 1280px, 820px and 360px, including a near-boundary review at 1360px after adopting the shared 1200px and 767px container breakpoints. Screenshot review and measured page widths found no horizontal page overflow; the bounded disposition was ship. These are browser viewport checks, not tests on physical devices. The follow-up changes layout only, with no new workflow or API behaviour.
