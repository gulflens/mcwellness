...
- [ ] **Step 1: Rewrite `tests/db/team.test.ts` first.** Un-skip Task 3's skipped cases and change who acts: every case that created, granted, suspended or reset **as the admin** now does it as the owner (`OWNER_AUTH`), and gains a twin asserting the admin is answered **403** and nothing changed. Then add, each its own `it`:
  ...
  4. `PATCH` that changes the email calls `h.authAdmin.setEmail` once with the new address (spy, as round 57's test does) and the row follows; with `setEmail` made to throw `EmailInUseError` the answer is 409 `email_in_use` and the row is **unchanged**; with the row update made to fail (send a `phone` of `'+0'` past zod by… no: instead spy `setEmail` and assert it is called a **second** time with the **old** address when the database refuses — provoke that by PATCHing an owner's row as the other owner with a changed name, which the trigger refuses).
