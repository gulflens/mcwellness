...
1. **Hosting** (not yet chosen). Must terminate TLS, forward the caller's
   ...
   Set `TRUSTED_PROXY_HOPS` to the number of proxies in front of the API so
   the rate limiter keys on the real caller; with 0, only the socket's
   address counts and the header is ignored. Strip query strings from
...
2. **Protective headers** on every answer (`app/api/_middleware/security.ts`):
   ...
   fonts and connections (`'self'`, images also `data:`), no framing
...
3. **Rate limits** (`app/api/_middleware/rate-limit.ts`), per minute, from
   ...
   API answers 429 with `Retry-After` and does no work. The counters live in
   the process's memory: right for one instance. Several instances need a
   shared store (Redis or Postgres) behind the same interface.
...
API keys, connection strings with passwords, assigned secrets; local-host
strings and the tests' documented fakes are the only allowances) and every
test, including `tests/security/`. CI adds `pnpm audit --prod` failing on a
