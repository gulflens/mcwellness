...
| **Money**: purchases, entitlements, invoices and their lines, payments, refunds, rendered invoice files | all | all | all | own schedule | all | own record, and not a minor's own login | `db/policies/billing/ledger.sql` (`ledger_readers`, `catalogue_readers`, `exception_readers`) and `db/policies/portal/money.sql` (`portal_money_adults`), erasure-gated. **`package_extension` stood here for two days and is gone.** Migration 410 (2026-09-10) added the table with `package_extension_readers` and `package_extension_writers`, and made it the seventh table `portal_money_adults` narrows; migration 412 (2026-09-12) dropped the table with the whole extension feature, on the operator's ruling that a programme's term is optional and a programme with no term has nothing to extend (`docs/SPEC/billing.md` section 4.3, `docs/CHANGE-REQUESTS/billing-11.md`). `portal_money_adults` names six tables again, which is what migration 702's own rollback block always listed. No cell's letters moved in either direction. A billing exception is the office's: `db/policies/billing/ledger.sql` gives it to O, A, L and F only. **The catalogue itself is not in the cells above**: `package`, `package_component` and `package_price` carry `catalogue_readers`, which is the four office roles — O, A, L and F — and neither a practitioner nor a household, whatever their cell says about their own money. A price list is a sales instrument and neither of them sells. **The API is narrower**: `billing.invoice.read` is the four office roles, and `actor.ts` says in the file that this is deliberate for now. |
...
carry no bypass setting, so undoing them is a migration's act. An owner's row
cannot be moved to another practice either, because a row that moved would leave
its ownership row behind and the lock would stop recognising itself. A working
role is taken away only by `app.revoke_staff_role`, which is the owners' and
leaves everyone with at least one working role.

**And ownership is not granted by the API role at all**, an owner's own request
included. `owner_grants_owner` and `owner_keeps_owner` in `role_guard.sql` are
unconditional as of 22 September 2026: they admitted an owner until the round's
