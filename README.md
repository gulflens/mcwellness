...
No real person is ever written into this repository, so `pnpm seed` invents a whole practice to build and test with: one studio, six people with logins (an owner who is also the lead practitioner, two more practitioners, a coordinator, and two households in the client portal), six services, and twenty clients with contacts, home locations and consents. The same data comes out every run. Every value sits in a range reserved for fakes: names from a fixed fictional list, phones in the `+971 50 000 xxxx` block, emails at `example.com`, Emirates IDs from `784-1900-*`, only on a parent who has consented, sealed with the key in `IDENTITY_KEY`. The two local placeholders (the sign-in secret and the identity key) are accepted only when `APP_ENV=development` is set explicitly. Running the command again adds nothing; `pnpm seed --fresh` wipes the local database and rebuilds it. `pnpm seed` refuses production always, and a Supabase project unless `APP_ENV=staging`. `pnpm seed:sql` renders the same practice as an SQL script for a project no connection string reaches: the script opens its own transaction, refuses a database that already holds a practice, stops if applied statement by statement, and names the environment it was rendered for, but it runs wherever it is pasted, so check the project before pasting.
