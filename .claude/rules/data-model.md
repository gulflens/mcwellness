...
- Standard-column exemptions, recorded 2026-09-02: `tenant` has no `tenant_id` (it is the tenant); `audit_log` keeps a bigint chain id and has no `updated_at`, `created_by` or foreign keys because it is append-only; `app.audit_chain` and `schema_migration` are single-purpose bookkeeping tables. Nothing else is exempt.
