 ...
 * collapsed to single spaces, trimmed and capped by code point. Pure.
 *
...
// the zero-width space (U+200B), the word joiner (U+2060) and the byte order mark
// (U+FEFF). The zero-width joiners (U+200C, U+200D) stay: Persian and Urdu spell with them.
const CONTROLS = new RegExp(
  `[${span(0x00, 0x08)}${span(0x0b, 0x0c)}${span(0x0e, 0x1f)}${span(0x7f, 0x9f)}${cp(0x200b)}${cp(0x2060)}${cp(0xfeff)}]`,
  'g',
);
...
export function cleanText(value: string, max: number): string {
  const cleaned = value
    ...
    .replace(/ {2,}/g, ' ')
    .trim();
  // Cut by code point, never through a surrogate pair.
