 ...
 * The resource policy on this path alone is `cross-origin`, so the browser
 * completes the beacon's opaque reply quietly instead of discarding it and
 * logging it as blocked; nothing new becomes readable — a fetch in CORS mode
 * is governed by the CORS answer above, and a beacon's reply is never exposed
 * to the page. It is set by `securityHeaders` (app/api/_middleware/security.ts),
 * which writes its headers after the handler and would overwrite one set here.
 */
