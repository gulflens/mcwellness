...
describe('what refuses a recording', () => {
  it('refuses nothing when the record, the person and the household all allow it', () => {
    expect(refusalsForRecording(context())).toEqual([]);
  });
  ...
  it('says only that the record is not one of this practice’s', () => {
    ...
    expect(refusalsForRecording(context({ clientFound: false }))).toEqual(['client_not_found']);
  });

  it('says only that the record is not on their schedule', () => {
    expect(refusalsForRecording(context({ visible: false }))).toEqual(['not_visible']);
  });
  ...
  it('gives every reason at once rather than one at a time', () => {
    ...
    expect(
      refusalsForRecording(context({ credentialOk: false, activeConsentPurposes: ['home_visit'] })),
  ...
  it('refuses a client with no date of birth rather than treating them as an adult', () => {
    ...
    expect(refusalsForRecording(context({ hasDateOfBirth: false }))).toEqual([
      'date_of_birth_unknown',
    ]);
  });

  it('asks for the guardian’s own agreement for a minor, and not otherwise', () => {
    expect(refusalsForRecording(context({ isMinor: true }))).toEqual([
  ...
  it('does not ask for a guardian’s agreement it cannot know is needed', () => {
    ...
    expect(refusalsForRecording(context({ hasDateOfBirth: false, isMinor: false }))).toEqual([
      'date_of_birth_unknown',
    ]);
  });

  it('refuses a recording with no person to attribute it to', () => {
    expect(refusalsForRecording(context({ practitionerId: null }))).toEqual([
      'no_practitioner_row',
    ]);
  });

  it('asks for the home-visit agreement always, because the brain map is a home service', () => {
    // No delivery mode is taken from the caller: the word `studio` would have
    // walked past this with nothing on the server to check it against.
    expect(refusalsForRecording(context({ activeConsentPurposes: ['participation'] }))).toEqual([
