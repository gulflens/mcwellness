 ...
 * taken with `<input capture>`, compressed on the device to at most 1 MB,
 * and digested so the server can recognise the same file later.
 *
 * The bytes stay on the device for now. The API's body cap is 64 KB and the
 * trunk's StorageProvider lands in shared-zone round 14; the upload door is
 * the change request in docs/CHANGE-REQUESTS/session-capture-02.md. What
 * travels today is what this returns: a size, a type and a digest, which is
 * what the `photo_captured` event carries and what the close route files as
 * a document.
 */
