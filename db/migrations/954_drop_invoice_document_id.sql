-- 954_drop_invoice_document_id.sql
-- The dead column goes (docs/PLAN/pieces-seven-to-nine.md, the last of the
-- small things; billing-04.md request 5; round 24 of this file's own notes).
--
-- **Why it is dead.** `invoice.document_id` was declared in 402 as "the
-- rendered PDF. Nullable, and written by nothing in this pull request."
-- Nothing has written it since, and nothing can: `invoice` grants no update at
-- all, so the column could only ever be filled at insert time, and the PDF
-- does not exist then — it is rendered from the row afterwards. Billing solved
-- the real need with `billing_document` (407), a link table with its own
-- idempotent filing function, which is what every rendered invoice and receipt
-- hangs off today.
--
-- Round 24 recorded what had to happen before this could: something still read
-- the column. Three things did, and all three are put right in the same commit
-- as this migration.
--
--   1. `app.erase_client` — arm (b) of its "which documents are kept"
--      question, reading `invoice.document_id` dynamically. It caught nothing,
--      by construction, and arm (c) over `billing_document` catches what it
--      was there to catch. Replaced below.
--   2. `tests/billing/db/packages.test.ts` — asserted the column stayed null,
--      which is a test that a dead column is dead. It now asserts what
--      actually holds: an invoice names no document at all and the rendered
--      PDF hangs off `billing_document`.
--   3. `tests/client/db/erasure_act.test.ts` — its fixture wrote the column on
--      the invoice it files, and one assertion read it back. Both go; the file
--      is kept through the erasure by its own kind, which is what was
--      happening anyway, and the link-table path is proved by that file's own
--      "a rendered tax document" case over `billing_document`.
--
-- **Replacing the function whole.** `create or replace` has no patch form, so
-- the body below is 107's verbatim with arm (b) removed and its comment
-- rewritten to say where it went. Every other step, key and comment is
-- untouched, including the guards that let this function run on a database
-- carrying only some of the streams' ranges. A future stream extending
-- `app.erase_client` copies **this** body, not 107's.
--
-- **Why the second half of the trunk's range.** It alters `invoice`, which is
-- billing's own table (docs/SPEC/OWNERSHIP.md), so it must sort last.
--
-- Needs: 402 (invoice, and the column), 407 (billing_document, which arm (c)
-- reads), 107 (the function this replaces)

------------------------------------------------------------------------------
-- 1. app.erase_client, 107's body without the arm that read the dead column.
------------------------------------------------------------------------------
...
$$;
------------------------------------------------------------------------------
-- 2. The column, and the index that only existed to serve it.
------------------------------------------------------------------------------
drop index if exists invoice_document_idx;
alter table public.invoice drop column document_id;

-- rollback:
--   alter table public.invoice add column document_id uuid;
--   alter table public.invoice add constraint invoice_document_fk
...
--   -- And re-create app.erase_client as 107_erase_report.sql defines it, with
--   -- arm (b) back in place.
