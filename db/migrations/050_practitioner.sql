...
-- Who delivers or supervises clinical work, and what each one is authorised
...
create table practitioner (
  ...
  dha_professional_licence_no   text,
...
-- The authorisation table: practitioner x jurisdiction x service type.
create table credential (
  id                    uuid primary key default gen_random_uuid(),
  tenant_id             uuid not null references tenant (id),
  practitioner_id       uuid not null references practitioner (id),
  jurisdiction          text not null,           -- DHA, DOH, MOHAP: open set
  service_type_id       uuid not null references service_type (id),
  licence_type          text not null,           -- dha_psychologist, dha_technician, ...: open set
  licence_number        text,
  certification         text,                    -- bcia_bcn, vendor_qeeg, ...: open set
  ...
  updated_at            timestamptz not null default now(),
  created_by            uuid references app_user (id),
  unique (practitioner_id, jurisdiction, service_type_id),
  constraint credential_valid_range check (valid_to > valid_from)
);
