-- 105_erasure_reaches_the_visit.sql
-- Needs: 100 (erasure_request, app.erase_client), 104 (the version of that
--        function this one replaces, and the request's own columns).
--
--        The visit tables (session, session_event, visit_actuals) and payment
--        belong to ranges this one may not name: db/runner refuses a `Needs`
--        naming a higher number, and docs/SPEC/OWNERSHIP.md is why — apply
--        order across the ranges is not fixed, and a database carrying this
--        range without session-capture's or billing's is an ordinary state of
--        affairs. So every statement below that reaches another stream's
--        table asks whether it is there first, exactly as 104 asks about
--        `invoice`. Where it is not, there is nothing of that stream's to
--        erase.
--
-- The erasure did not reach the visit, and the letter said it did.
--
-- Four reviews of pull request 51 agreed on the headline. `app.erase_client`
-- anonymised the client, the contacts, the addresses, the goals and the
-- documents, and touched nothing in the 300-series at all — so after an
-- erasure the practice still held the coordinate its practitioner stood at
-- when they checked in and out (the household's own door, while the same
-- function was reducing their address to the middle of an emirate), the
-- observations written after each visit, the reason a record was amended,
-- the access notes from the drive, and whatever a device had put in an
-- event's payload. This adds the step that reaches them.
--
-- It also does three smaller things the same reviews asked for.
--
--   * `session.setup_photo_document_id` is unlinked. Its foreign key to
...
--     reference identifying a person's account is not.
--   * The request row says who performed the erasure and when its own record
--     stops being kept, and a guard trigger says what may be written to it
--     afterwards.
--
-- What is deliberately kept, and named in the letter: `pre_rating`,
-- `post_rating`, `telemetry`, `preflight`, `signal_check` and
-- `signal_quality_score`. They are numbers and checklist keys — measurements
-- with nobody attached to them once the record around them is anonymous — and
-- the practice needs them to see its own work in aggregate.

alter table erasure_request
  add column performed_by    uuid references app_user (id),
  -- Delivery is a person handing the letter over on WhatsApp, so only a
  -- person can say it happened. The sweep reads this: the number the
  -- confirmation went to is kept until the letter has gone and the files are
  -- confirmed clear, and not one day longer.
  add column letter_sent_at  timestamptz,
  add column retention_until timestamptz;

create index erasure_request_performed_by_idx on erasure_request (performed_by);

comment on column public.erasure_request.performed_by is
  'Who carried the erasure out, beside requested_by_contact_id, who asked for it.';
comment on column public.erasure_request.letter_sent_at is
  'When the confirmation letter was handed to the household. Set by the practice when '
  'it sends it; requested_by_phone is cleared once this and files_cleared_at both stand.';
comment on column public.erasure_request.retention_until is
  'Five years from the erasure (CLAUDE.md rule 8). The record that an erasure happened '
  'outlives the record it ended, and then goes the way of everything else.';

------------------------------------------------------------------------------
-- The write floor on a closed request.
--
-- db/policies/client/writers.sql lets the owner, an admin and the lead
-- practitioner update an erasure_request, on the premise stated in its own
-- comment: app.erase_client fills in what happened as the table owner, so
-- app_role never needs to. That premise was true when it was written and is
-- not any more — the route writes the letter's id and version, and the
-- practice stamps when it sent it — so the policy's open update is now an
-- open door onto `summary`, `reason` and `performed_at`, the row that exists
-- to say what an erasure did.
--
-- Row security asks which rows; this asks which columns, so it is a guard
-- trigger in the pattern of app.guard_location_notes() and migration 903.
-- Two updates are admitted and no others:
--
--   a) closing the row — performed_at moving from null, which is
--      app.erase_client's own last statement and happens once;
--   b) the letter columns afterwards: which document it is, which version of
--      the wording it was cut from, and when it was handed over.
--
-- Like both of its precedents it stands aside when no role has been assumed
-- at all: that is the owner's own maintenance — a migration, the sweep job,
-- the seed — never a request through the API, which always stamps a role.
------------------------------------------------------------------------------
create function app.guard_erasure_request_write() returns trigger
language plpgsql
...
begin
  ...
  -- (b) Afterwards, only the letter. Structural rather than an enumerated
  -- list of what may not change, so a column added to this table later is
  -- guarded by this trigger rather than slipping past it.
  ...
  then
    raise exception 'an erasure request records what happened and is not rewritten'
      using errcode = 'insufficient_privilege',
            hint    = 'Only the confirmation letter and when it was sent may be written afterwards.';
  end if;
  return new;
end
$$;
...
create trigger guard_erasure_request_write before update or delete on public.erasure_request
  for each row execute function app.guard_erasure_request_write();

------------------------------------------------------------------------------
-- app.erase_client(), 104's body with the visit step added, the payment
-- reference cleared, the setup photo unlinked, and the request row saying who
-- performed it and how long it is kept. Read it as a diff of those, not as a
-- new function: same signature, same security definer, same pinned
-- search_path, same steps in the same order, same summary keys plus four.
------------------------------------------------------------------------------
...
$$;
-- rollback:
--   re-create the function as 104_erasure_the_act.sql defines it (no visit
--   step, no payment reference, no setup photo unlink, and no performed_by or
--   retention_until in the closing update), then:
--   drop trigger if exists guard_erasure_request_write on public.erasure_request;
--   drop function if exists app.guard_erasure_request_write();
--   drop index if exists erasure_request_performed_by_idx;
--   alter table erasure_request
