-- 964_erasure_reaches_concerns_and_health.sql
-- The erasure reaches what a household was worried about and what they told
-- the practice about their health (108_concerns_and_health.sql).
--
-- **Why this file is in the trunk's range and not the client record's.**
-- `app.erase_client` belongs to the client record, and every other extension of
-- it — 104, 105, 106, 107 — sits in that range. But 954, a trunk migration,
-- redefined the function last, and a database applies migrations in numeric
-- order: a redefinition at 108 is overwritten by 954 a moment later, and its
-- steps vanish with no error and no output. That was not reasoned out in
-- advance; it was written at 108 first, and
-- tests/client/db/concerns_and_health.test.ts failed against a fresh database
-- with a summary carrying every key except the two new ones.
--
-- So the rule, for whoever extends this function next: **it must be numbered
-- above the highest migration that defines it**, whatever range that lands in.
-- Today that is 954. Recorded in docs/CHANGE-REQUESTS/trunk-notes.md.
--
-- This is 954's body with two steps and two summary keys added, and is meant to
